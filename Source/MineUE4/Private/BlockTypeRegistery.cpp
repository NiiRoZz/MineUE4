...
void ABlockTypeRegistery::DrawAtlasElement(UCanvas* canvas, UTexture2D* texture, uint32 currTextureIdx, const FVector2D& atlasElemSize)
{
	const uint32 indexX = currTextureIdx % (uint32)atlasElemSize.X;
	const uint32 indexY = currTextureIdx / (uint32)atlasElemSize.X;

	canvas->DrawTile(texture, (float)indexX * (float)m_TextureSize, (float)indexY * (float)m_TextureSize, (float)m_TextureSize, (float)m_TextureSize, 0, 0, texture->GetSizeX(), texture->GetSizeY(), EBlendMode::BLEND_Opaque);
}
...
void ABlockTypeRegistery::DrawTextureAtlas()
{
	...
	UKismetRenderingLibrary::ClearRenderTarget2D(this, m_RenderTarget, FLinearColor::Transparent);
	...
	for (int32 i = 0; i < m_BlockTypes.Num(); ++i)
	{
		if (m_BlockTypes[i].Visibility != EBLockVisibility::OPAQUE)
		{
			continue;
		}
